...
static int ps_config(oconfig_item_t *ci) {
  ...
  _Bool *detail_flags[] = {
    ...
    &want_detail_configuration_g.num_fd,
    &want_detail_configuration_g.num_maps,
  ...
  for (int i = 0; i < ci->children_num; ++i) {
    ...
    if (strcasecmp(c->key, "Process") == 0) {
      ...
      assert (STATIC_ARRAY_SIZE(stat_names) == STATIC_ARRAY_SIZE(detail_flags));
...
static void dispatch_value_helper(
    value_list_t *vl, const char *type, const char *type_instance,
    int values_len, _Bool doing_detail, _Bool want_detail)
{
    ...
    derive_t cswitch_invol)
{
  ...
  if (doing_detail) {
    ...
    if (command_line != NULL) {
      meta_data_add_string(vl.meta, "processes:command_line", command_line);
    }
  }

  vl.values[0].gauge = num_proc;
  vl.values[1].gauge = num_lwp;
  dispatch_value_helper(&vl, "ps_count", NULL, 2, doing_detail,
                        config->ps_count);
  ...
  if (num_fd > 0) {
    ...
    dispatch_value_helper(&vl, "file_handles", NULL, 1, doing_detail,
                          config->num_fd);
  }

  if (num_maps > 0) {
    vl.values[0].gauge = num_maps;
    dispatch_value_helper(&vl, "file_handles", "mapped", 1, doing_detail,
                          config->num_maps);
  }

  if ((cswitch_vol != -1) && (cswitch_invol != -1)) {
    vl.values[0].derive = cswitch_vol;
    dispatch_value_helper(&vl, "contextswitch", "voluntary", 1, doing_detail,
                          config->cswitch_vol);
    vl.values[0].derive = cswitch_invol;
    dispatch_value_helper(&vl, "contextswitch", "involuntary", 1, doing_detail,
  ...
  DEBUG(
      "name = %s; pid = %s; num_proc = %lu; num_lwp = %lu; num_fd = %lu; num_maps = %lu; "
      ...
      instance_name, pid, num_proc, num_lwp, num_fd, num_maps,
      ...
      io_syscw, io_diskr, io_diskw, cswitch_vol,
      cswitch_invol);
} /* void ps_submit_proc_stats */

#undef MAX_VALUE_LIST_SIZE


static void ps_submit_procstat_entry(
    const char *instance_name, procstat_entry_t *entry) {
